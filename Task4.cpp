...
#pragma omp section{
		...
		team.erase(remove(team.begin(), team.end(), a), team.end());
	
		n = team.size();

		/*	for (size_t i = 0; i < team.size(); i++)
			{
				std::cout << "\tMonk with energy " << team[i] << std::endl;
			}*/
	}
	if (team[0] > team[1])
	{
		std::cout << "Win monk with energy " << team[0] << std::endl;
	}
	else {
		std::cout << "Win monk with energy " << team[1] << std::endl;
	}
